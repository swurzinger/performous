...
void FFmpeg::open() {
	...
	case AVMEDIA_TYPE_AUDIO:
		...
		av_opt_set_int(m_swrContext, "in_channel_layout", m_codecContext->channel_layout ? m_codecContext->channel_layout : av_get_default_channel_layout(m_codecContext->channels), 0);
		av_opt_set_int(m_swrContext, "out_channel_layout", av_get_default_channel_layout(AUDIO_CHANNELS), 0);
		av_opt_set_int(m_swrContext, "in_sample_rate", m_codecContext->sample_rate, 0);
		av_opt_set_int(m_swrContext, "out_sample_rate", m_rate, 0);
		av_opt_set_int(m_swrContext, "in_sample_fmt", m_codecContext->sample_fmt, 0);
		av_opt_set_int(m_swrContext, "out_sample_fmt", AV_SAMPLE_FMT_S16, 0);
		swr_init(m_swrContext);
		if (!m_swrContext) throw std::runtime_error("Cannot create resampling context");
...
void FFmpeg::operator()() {
	...
	if (m_swrContext) swr_close(m_swrContext);
...
void FFmpeg::processAudio(AVFrame* frame) {
	...
	int16_t *output;
	int out_linesize;
	int out_samples = av_rescale_rnd(swr_get_delay(m_swrContext, m_rate) +
		frame->nb_samples, frame->sample_rate, m_rate, AV_ROUND_UP);
	av_samples_alloc((uint8_t**)&output, &out_linesize, AUDIO_CHANNELS, out_samples, AV_SAMPLE_FMT_S16, 0);
	out_samples = swr_convert(m_swrContext, (uint8_t**)&output, out_samples, (const uint8_t**)&frame->data[0], frame->nb_samples);
